...
class QueueManager(QObject):
    ...
    def _parse_output_line(self, line: str):
        ...
        if m:
            ...
            if "language" in info and info["language"] != new_lang:
                # Language changed between fast-detect and Whisper fallback.
                # The first (wrong) detection likely caused the fallback.
        ...
        if "Reference pipeline recovered" in clean:
            ...
            # Clear the fallback flag — we recovered successfully
        ...
        # This is the definitive output folder (parent of the .txt file).
        # We prefer this over "Creating output folder" which fires for
        # every subfolder including cache/ and download temp dirs.
        ...
        # Always overwrite: UltraSinger prints this for cache, download,
        # AND song output folders; the last one before the UltraStar file
        # line is the song output folder.
        m = re.search(r"Creating output folder\.\s*->\s*(.+)", clean)
        if m:
            folder = m.group(1).strip()
            # Skip cache folders — they are deleted after conversion
    ...
    def _on_item_finished(self, exit_code: int):
        ...
        if info.get("pipeline") == "reference":
            ...
            # Mark language as cause only if it actually changed
            # (e.g. fast-detect said "cy" but Whisper later said "en")
            if info.get("language_changed"):
                info["language_caused_fallback"] = True
        elif info.get("lrclib_result") == "instrumental":
